    ...
    <div class = "sticky2">
        <?php if($this->request-> getSession()->read('Auth.User.adminfrag') != 1): ?>
            <?= $this->Form->create(__("View"),
                ["type" => "post","id" => "editform","url" => ["action" => "register2", "?" => array("staff_id" => $user_id, "year" => $year, "month" => $month)]],
                array('target' => '_blank')); ?>
        <?php elseif($adminfrag == 1): ?>
            <?= $this->Form->create(__("View"),
                ["type" => "post","id" => "editform","url" => ["action" => "register", "?" => array("staff_id" => $user_id, "year" => $year, "month" => $month)]],
                array('target' => '_blank')); ?>
    ...
    <tr class = "hvrow">
        ...
        <?php if($this->request-> getSession()->read('Auth.User.adminfrag') == 1): ?>
            ...
            if((empty($results[$i]["bikou"]))) {
                echo $this -> Form -> control("bikou[$i]",array("type" => "text","label" => false));
            } else {
                echo $this -> Form -> control("bikou[$i]",array("type" => "text","label" => false,"value"=>$results[$i]["bikou"]));
            }
            ?>
        <?php else: ?>
