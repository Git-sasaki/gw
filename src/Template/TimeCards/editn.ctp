...
<script type="text/javascript">
    $(function() {
        ...
        btn.addEventListener('click', function() {
            ...
            if(pass == "redmoon") {
                location.href = "delete?year="+<?=$year?>+"&month="+<?=$month?>+"&user_id="+<?=$user_id?>;
            } else {
                alert("パスワードが違います");
            }
        })
    ...
    function sgChange(obj) {
        allctrlByObj = document.getElementsByName(obj.name);
        trg = 0;
        for (let i = 1; i <= allctrlByObj.length; i++) {           
            if ((allctrlByObj[i].type === "select-one") && (allctrlByObj[i].name == obj.name)) { 
                trg = i;
                break;
            }
        }
        ...
        if (obj.checked) {
            allctrlByObj[trg].disabled = null;
            options = allctrlByObj[trg].options;
            options[1].selected = true;
        } else {
            allctrlByObj[trg].value = 0;
            allctrlByObj[trg].disabled = "disabled";
        }
    }
    ...
    function OnEnter(obj) {
        ...
        trg = 0;
        for (let i = 1; i <= allctrlByObj.length; i++) {           
            if ((allctrlByObj[i].type === "checkbox") && (allctrlByObj[i].name == obj.name)) { 
                trg = i;
                break;
            }
        }

        if (allctrlByObj[trg].checked) {
            if ( obj.value == 0) {
    ...
    //登録ボタン押下時の昼食・送迎のチェック（利用者）
    function OnTouroku2() {
        const inputs = document.querySelectorAll('input[name^="intime["]');

        for (let i = 1; i <= inputs.length; i++) {           
            //休みチェック
            const kekkin1 = document.getElementsByName('koukyu[' + i + ']');
            const kekkin2 = document.getElementsByName('paid[' + i + ']');
            const kekkin3 = document.getElementsByName('kekkin[' + i + ']');
            const zaitaku =  document.getElementsByName('remote[' + i + ']');
            if ( kekkin1[1].checked || kekkin2[1].checked || kekkin3[1].checked) {
                //食事チェック    
                const checkboxes = document.getElementsByName('meshi[' + i + ']');
                if ( checkboxes[1].checked) {
                    alert( "公休・有休・欠勤のいづれかが設定されているので" + '\n' + "食事のチェックを外して下さい。");
                    const target = document.getElementById('jump' + (i - 2));
                ...
                //送迎チェック    
                const checkboxes1 = document.getElementsByName('ou[' + i + ']');
                const checkboxes2 = document.getElementsByName('fuku[' + i + ']');
                
                if ( checkboxes1[1].checked || checkboxes2[1].checked) {
                    alert( "公休・有休・欠勤のいづれかが設定されているので" + '\n' + "送迎（往・復）のチェックを外して下さい。");
                    const target = document.getElementById('jump' + (i - 2));
                    target.scrollIntoView({ behavior: 'smooth' }) ;
            ...
            if ( zaitaku[1].checked) {
                //欠勤チェック    
                if ( kekkin1[1].checked || kekkin2[1].checked || kekkin3[1].checked) {
                    alert( "在宅が設定されていていますが"  + '\n' + "公休・有休・欠勤のいづれかも設定されています。" + '\n' + "確認して下さい。");
                    const target = document.getElementById('jump' + (i - 2));
                    target.scrollIntoView({ behavior: 'smooth' }) ;
...
<div class = "odakoku">
    ...
    <div class = "main2 pt15">
        <?php if($user_id == 12): ?>
            <h4 class = "midashih4 mlv25 mb10">
                ...
                <span style = "font-size: 16px; margin-left:10px;">
                    ...
                    <?= $this->Html->link('[門脇さんコマンド]', ['action'=>'schedule', "?" => ["id" => $user_id, "year" => $year, "month" => $month]]); ?>
                </span>
            </h4>
        ...
        <?php if($this->request-> getSession()->read('Auth.User.adminfrag') != 1): ?>
            <?= $this->Form->create(__("View"),
                ["type" => "post","id" => "editform","url" => ["action" => "register2", "?" => ["staff_id" => $user_id, "year" => $year, "month" => $month]]],
                ['target' => '_blank']); ?>
        <?php elseif($adminfrag == 1): ?>
            <?= $this->Form->create(__("View"),
                ["type" => "post","id" => "editform","url" => ["action" => "register", "?" => ["staff_id" => $user_id, "year" => $year, "month" => $month]]],
                ['target' => '_blank']); ?>
    ...
    <tr class = "hvrow">
        ...
        ?>
        <td style = "color:<?= $color?>" class = "sema" id = "jump<?=$i?>"><?= $i; ?></td>
        <td style = "color:<?= $color?>" class = "sema"><?= $weekList[date("w",$timestamp)]; ?></td>
        ...
        <?php else: ?>
            ...
            <?php 
                if(empty($results[$i]["ou"])){
                    echo $this -> Form -> control("ou[$i]",["type" => "checkbox","label" => false,"value" => 1]);
                }else{
                    echo $this -> Form -> control("ou[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
                }
            ?>
            </td>
            <td class = "sema antisp">
                ...
                if(empty($results[$i]["fuku"])){
                    echo $this -> Form -> control("fuku[$i]",["type" => "checkbox","label" => false,"value" => 1]);
                }else{
                    echo $this -> Form -> control("fuku[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
                }?>
            </td>
            <!--
            <td class = "sema antisp">
                ...
                if(empty($results[$i]["medical"])){
                    echo $this -> Form -> control("medical[$i]",["type" => "checkbox","label" => false,"value" => 1]);
                }else{
                    echo $this -> Form -> control("medical[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
                }?>
            </td>
            -->
            <?php endif; ?>
            <td class = "sema antisp">
                ...
                if(empty($results[$i]["meshi"])){
                    echo $this -> Form -> control("meshi[$i]",["type" => "checkbox","label" => false,"value" => 1]);
                }else{
                    echo $this -> Form -> control("meshi[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
                }?>
            </td>
            <!--<td class = "sema antisp">-->
            <td class = "futu antisp">
                ...
                if(empty($results[$i]["support"])){
                    echo $this -> Form -> control("support[$i]",["type" => "checkbox","label" => false,"value" => 1,"onchange" =>"sgChange(this)"]);
                    echo $this->form->input( "support[$i]",array('disabled' => "disabled",'label'=>false,'type' => 'select', 'options' => $workName, 'value' => '0',"onchange" =>"OnEnter(this)")); 
                 }else{
                    echo $this -> Form -> control("support[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true,"onchange" =>"sgChange(this)"]);
                    echo $this->form->input( "support[$i]",array('label'=>false,'type' => 'select', 'options' => $workName, 'default' => $results[$i]["support"],"onchange" =>"OnEnter(this)")); 
                }
                ?>
            </td>
        <td class = "sema">
        <?php 
            if(empty($results[$i]["koukyu"])){
                echo $this -> Form -> control("koukyu[$i]",["type" => "checkbox","label" => false,"value" => 1]);
            }else{
                echo $this -> Form -> control("koukyu[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
            }?>
        </td>
        <td class = "sema">
            <?php 
            if(empty($results[$i]["paid"])){
                echo $this -> Form -> control("paid[$i]",["type" => "checkbox","label" => false,"value" => 1]);
            }else{
                echo $this -> Form -> control("paid[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
            }?>
        </td>
        <td class = "sema antisp">
            <?php 
            if(empty($results[$i]["kekkin"])){
                echo $this -> Form -> control("kekkin[$i]",["type" => "checkbox","label" => false,"value" => 1]);
            }else{
                echo $this -> Form -> control("kekkin[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
            }?>
        </td>
        ...
        <td class = "sema">
            ...
            if(empty($results[$i]["remote"])){
                echo $this -> Form -> control("remote[$i]",["type" => "checkbox","label" => false,"value" => 1]);
            }else{
                echo $this -> Form -> control("remote[$i]",["type" => "checkbox","label" => false,"value" => 1,"checked" => true]);
            }?>
        </td>
        <?php if($this->request-> getSession()->read('Auth.User.adminfrag') != 1): ?>
            <td class = "futu antisp">
        ...
        <?php elseif($adminfrag == 0): ?>
            ...
            <?php
                if((empty($results[$i]["user_staffid"]))){
                    echo $this->Form->select("user_staffid[$i]",$admresults,['empty'=>['0'=>null]]);
                } else {
                    echo $this->Form->select("user_staffid[$i]",$admresults,['empty'=>['0'=>null],'value'=>$results[$i]["user_staffid"]]);
                }
            ?>
            </td>
        <?php endif; ?>
...
<div class = "alltable2 antisp">
    ...
    <div class = "odakoku mt-5 spmargin">
        <div class="ml10_button mlv25">
            ...
            <?php else: ?>
                <?= $this->Form->button("登録",array('onClick' => 'return OnTouroku1()')) ?>
                <!--<?= $this->Form->button(__("登録")) ?>-->
            <?php endif; ?>
        </div>
        <div class="newright3">
            <?= $this->Html->link(__('[勤務情報の詳細]'), ['action' => 'detailn', "?" => ["id" => $user_id, "year" => $year, "month" => $month]]) ?>
        </div>
    </div>
<?php $this -> Form -> end(); ?>
</div>